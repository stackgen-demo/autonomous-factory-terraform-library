...
- Application build context, Dockerfile, optional test command, deployment manifest path, and Backstage catalog path.
...
Use `templates/harness/aws-eks-dev-pipeline-v2.yaml.tftpl` as structural guidance. Adapt its placeholders from the application contract. Preserve native `TerraformPlan`, `TerraformApply`, and `K8sRollingDeploy` steps and the immutable `commit_sha` input. A successful rolling deployment is the delivery success criterion.
